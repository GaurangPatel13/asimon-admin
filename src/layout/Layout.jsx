...
import { FaHome, FaSignOutAlt, FaUserEdit, FaUserGraduate, FaWallet } from "react-icons/fa";
...
import { MdAutoGraph, MdOutlineCategory, MdOutlineRequestQuote } from "react-icons/md";
...
const Layout = () => {
  ...
  const menuItems = [
    ...
    {
      path: Routers.allFranchiseeList,
      label: "All Franchisee List",
      icon: <FaUserPlus />,
      // subRoutes: [],
    },
  ];
  ...
  return (
    <>
      {loading && <PageLoader />}
      <div style={{ backgroundImage: `url(${img})`, backgroundSize: "cover", backgroundPosition: "center" }} className="flex justify-end md:p-2 gap-4 w-full h-screen  bg-bg-color1">
        <div
        ...
        >
          <div className="py-4 text-center flex items-center justify-between w-full gap-4">
            <Link to={Routers.Dashboard}><img src={MainContent.logo} alt="Bionova Logo" className=" w-full h-10 object-contain" /></Link>
            <button
          ...
          <nav className="scrollbar-left flex-1 overflow-y-auto">
            <ul className="space-y-2 py-2 ">
              {menuItems.map(({ path, label, icon, subRoutes }) => {
                ...
                const allFranchiseeList = path === Routers.allFranchiseeList;
                ...
                return (
                  <li key={label}>
                    <button
                      onClick={() => {
                        if (isDashboard || distributionForm || addNews || allFranchiseeList) {
                          navigate(path);
                        } else {
                    ...
                    >
                      ...
                      {!isDashboard && !distributionForm && !addNews && !allFranchiseeList && (isOpen ? <IoIosArrowDown /> : <IoIosArrowForward />)}
                    </button>
          ...
          <div className="py-2 space-y-1">
            ...
            {actionButtons.map(({ label, icon, bgColor, func }, index) => (
              <div key={index} className="p-2">
                ...
                >
                  <div className={`p-2 ${bgColor} text-white rounded-lg text-base`}>{icon}</div>
                  {label}
                </button>
              </div>
            ))}
